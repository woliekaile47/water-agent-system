...
“预鉴”是一套道路积水监测研究系统：**无雨时用激光雷达记录道路高低，
下雨时用摄像头判断哪里有水，再根据道路地形计算水位、水深、面积和体积。**

当前版本已经完成合成仿真环境中的自动闭环和比赛演示，但还没有完成真实道路长期验证，
所以它是“合成数据旁路演示版”，不是已经投入使用的正式预警产品。

### 为什么要分成两个阶段

1. **无雨阶段**：LiDAR扫描干燥道路，得到`ground DEM`，可以理解为一张精确记录道路高低的地图。
2. **降雨阶段**：摄像头连续拍摄道路；时序模块自动寻找疑似积水位置并给SAM 2提示，SAM 2负责分出水域边界。
3. **水深计算**：系统把水域岸线映射到ground DEM，估计水面高度，再用“水面高度－道路高度”计算每个位置的水深。
4. **安全门控**：结果不可信时直接拒绝，不允许进入正式预警链路。

```text
干燥道路 LiDAR -> ground DEM
降雨视频 -> 自动提示 -> SAM 2水域候选 -> 连续帧传播
-> 岸线与DEM -> 水位/水深/面积/体积 -> 质量门控 -> 旁路Dashboard
```
...
| 模块 | 当前作用 | 是否为主算法 |
|---|---|---|
| 动态降雨视觉仿真 | 生成可重复的仿真道路、雨滴、飞溅和涟漪数据，用于研发和测试 | 否，只是离线研发工具 |
| 早期时序规则mask | 作为历史对照和消融实验 | 否，不作为最终水域分割 |
| 时序自动提示 | 自动生成SAM 2的框、正点和负点，避免人工逐帧点击 | 是，负责“去哪里找水” |
| SAM 2 | 生成水域候选mask并在连续视频中传播 | 是，主要视觉分割器 |
| 岸线与ground DEM | 根据水域边界反演水位、水深、面积和体积 | 是，主要几何计算模块 |
| quality gate | 拒绝不可信结果，并区分相机可见估计和全场景估计 | 是，安全模块 |

### 目前已经做到哪一步

| 项目 | 当前状态 |
|---|---|
| 5/10/20/40 cm仿真道路和动态降雨数据 | 已完成 |
| 时序模块自动生成SAM 2提示 | 已完成，不需要人工逐帧点击 |
| SAM 2连续41帧双向传播 | 已完成 |
| 岸线到DEM、水位、水深、面积和体积 | 已完成 |
| 多水深、多雨强、多seed合成验证 | 已完成 |
| 质量门控与Dashboard旁路监控 | 已完成 |
| 比赛仿真道路专用演示页面 | 已完成并人工验收 |
| 真实道路和真实降雨验证 | 未完成 |
| 多摄像头全域覆盖 | 未完成，后续方向 |
| 正式S5-S8预警启用 | 未启用 |

Seed 303最终确认包含12条序列、492帧。候选门控放行361帧、拒绝131帧；
在该合成确认集中，361个放行帧的水位误差均小于3 cm，没有观察到错误放行。
5 cm浅水仍然最困难；40 cm场景存在单摄像头看不到的第二积水盆地，因此只能报告相机可见范围结果。

### 我是软著或论文同学，应该先看什么

- **软著材料负责人**：先看[中文团队交接指南](docs/team_handoff_guide_zh.md)中的“软著材料”部分，再看`src/`、`scripts/`和Dashboard。
- **论文负责人**：先看[中文团队交接指南](docs/team_handoff_guide_zh.md)中的“论文材料”部分，再看`docs/experiments/`和Seed 303实验说明。
- **比赛演示负责人**：运行`bash scripts/run_phase2d_c10_competition_demo.sh`，只展示仿真道路页面。
- **代码复核人员**：使用标签`v1.1-competition-synthetic-shadow-demo`和commit `54238c7`，不要用未冻结的临时输出代替实验结果。

### 必须避免的错误表述

- 不能说“已经完成真实道路部署”；
- 不能说“所有水深都能稳定识别”，5 cm仍然困难；
- 不能把仿真Ground Truth当成算法输入；
- 不能把相机看不到的第二盆地自动补全为真实水域；
- 不能把SAM 2、PyTorch、ROS 2或Gazebo第三方源码写成本项目原创代码；
- 不能把旁路Dashboard称为已经启用的正式预警系统。

下面保留英文技术说明、旧S1–S8工程链路和历史实验，供开发与复核使用。
